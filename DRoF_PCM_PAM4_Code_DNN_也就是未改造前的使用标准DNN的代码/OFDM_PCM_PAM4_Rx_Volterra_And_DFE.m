% =========================================================================
% DRoF - OFDM_PCM_PAM4_Rx_Unified.m
% 目的：合并 Volterra 和 DFE 接收端脚本
%       一次运行同时执行两种均衡器，并分别保存结果到不同路径
%
% 流程:
% 1. 基础设置 (加载测试集，配置所有参数)
% 2. 加载测试参数 (PRBS23)
...
% 6. 执行 Volterra 均衡 -> 得到 received_eq_volterra
% 7. 执行 DFE 均衡      -> 得到 received_eq_dfe
% 8. 循环后处理 (针对两种结果分别执行 PCM盲同步 + OFDM解调 + BER计算)
% =========================================================================
clc;clear;close all;
...
run_mode       = 'test';        % 运行模式: 'test'
% --- 训练集 ---
train_prbs_name = 'PRBS23';
train_mode_name = 'train';
received_optical_power = -20;   % 设置接收光功率 (例如 -18)
suduandlength = '10Gsyms_20km'; % 速率和距离标识
quant = 8;                      % 量化比特数

% --- 1.2 Volterra 均衡器参数 ---
L_linear    = 41;   % 线性项记忆长度
L_nonlinear = 11;   % 二阶非线性记忆长度
lambda      = 1e-6; % 岭回归正则化系数

% --- 1.3 DFE 均衡器参数 ---
nFfeTaps    = 31;   % FFE 前馈抽头数
nDfeTaps    = 5;    % DFE 反馈抽头数
step_size   = 0.002;% LMS 步长
...
% 注意：图片和报告的保存路径将在后续循环中根据当前模型动态生成
...
% ---------------- [保存原始眼图] (Common) ----------------
% 图片保存到临时变量，后续根据不同模型复制，或者这里直接存到一个Common文件夹
% 为了简化逻辑，我们在后续循环中再绘制均衡器特定的图，这里是Raw Eye
common_image_path = fullfile(root_base_dir, 'RX_Matlab_Result_Images_png', 'Common_Raw');
if ~exist(common_image_path, 'dir'), mkdir(common_image_path); end
...
h_eye = gcf;
eye_filename = sprintf('Eye_Raw_%s_%s_%ddBm.png', prbs_base_name, run_mode, received_optical_power);
full_eye_path = fullfile(common_image_path, eye_filename);
...
% 2. RMS 功率归一化 (使信号功率为1)
test_input_common = rx_ac / rms(rx_ac); % 这是一个通用的 test_input

% --- [4.7] 对齐理想标签 ---
% 确保 test_input_common 和 original_pam_test_data 长度严格一致
current_len = length(test_input_common);
if current_len < total_pam_symbols
    original_pam_test_data = PAM_code(1:current_len);
elseif current_len > total_pam_symbols
    test_input_common = test_input_common(1:total_pam_symbols);
    original_pam_test_data = PAM_code;
else
    original_pam_test_data = PAM_code;
end

fprintf('测试集预处理完成。有效样本数: %d\n', length(test_input_common));
...
if lag_diff > 0
    ...
    train_input_common = raw_train_input(1+lag_diff : end);
    train_label_common = raw_train_label(1 : end-lag_diff);
elseif lag_diff < 0
    % Rx超前: 截去Rx尾部，截去Label头部
    lag_diff_abs = abs(lag_diff);
    train_input_common = raw_train_input(1 : end-lag_diff_abs);
    train_label_common = raw_train_label(1+lag_diff_abs : end);
else
    % 完美对齐
    train_input_common = raw_train_input;
    train_label_common = raw_train_label;
end

% 最终强制长度对齐 (取交集)
final_len = min(length(train_input_common), length(train_label_common));
train_input_common = train_input_common(1:final_len);
train_label_common = train_label_common(1:final_len);

% 通用训练数据准备完毕
disp(['训练数据准备完毕。最终样本数: ', num2str(length(train_input_common))]);

%% === 6. Volterra 均衡器 (Volterra Equalizer) ===
disp('----------------------------------------');
disp('=== 开始 Volterra 均衡器处理 ===');

% --- [6.0] 准备数据 ---
% 确保是列向量
train_input_vol = train_input_common(:);
train_label_vol = train_label_common(:);
test_input_vol  = test_input_common(:);

% 设置保存路径
image_save_path_vol = fullfile(root_base_dir, 'RX_Matlab_Result_Images_png', 'Volterra');
if ~exist(image_save_path_vol, 'dir'), mkdir(image_save_path_vol); end

% --- [6.1] 均衡器参数配置 (使用顶部配置的全局参数) ---
...
% --- [6.2] 构建训练矩阵 H_train ---
% 计算有效训练样本数
N_train_samples = length(train_input_vol) - (L_linear - 1);
% 截取对应的理想标签 (取线性窗口的中心位置作为参考点)
target_train = train_label_vol(center_linear : center_linear + N_train_samples - 1);

% 1. 构建线性特征矩阵 (H1)
H1_train = zeros(N_train_samples, L_linear);
for i = 1:N_train_samples
    % 截取线性窗口: [x(n)...x(n+L1-1)]
    H1_train(i, :) = train_input_vol(i : i + L_linear - 1).';
end
...
for i = 1:N_train_samples
    ...
    win_nl = train_input_vol(i + offset_idx : i + offset_idx + L_nonlinear - 1);
    col_idx = 1;
    % 双重循环生成交叉项
    for j = 1:L_nonlinear
...
fprintf('Volterra 训练完成。训练集 MSE: %.6f\n', mse_train);
...
% --- [6.4] 应用于测试集 (Testing) ---
% 构建测试集的线性项
N_test_samples = length(test_input_vol) - (L_linear - 1);

% 1. 测试集 H1
H1_test = zeros(N_test_samples, L_linear);
for i = 1:N_test_samples
    H1_test(i, :) = test_input_vol(i : i + L_linear - 1).';
end
...
for i = 1:N_test_samples
    % 同样的 offset 对齐逻辑
    win_nl = test_input_vol(i + offset_idx : i + offset_idx + L_nonlinear - 1);
    col_idx = 1;
    for j = 1:L_nonlinear
        for k = j:L_nonlinear
...
% 应用训练好的系数    均衡输出！！！
received_eq_volterra = (H_test * Volterra_Coeffs).'; % 转置为行向量 
% received_eq就是最终均衡后的信号，消除了色散和非线性失真的PAM4符号序列。




%% === 7. DFE 均衡器 (DFE Equalizer) ===
disp('----------------------------------------');
disp('=== 开始 DFE 均衡器处理 ===');

% --- [7.0] 准备数据 ---
% 确保是列向量
train_input_dfe = train_input_common(:);
train_label_dfe = train_label_common(:);
test_input_dfe  = test_input_common(:);

% 设置保存路径
image_save_path_dfe = fullfile(root_base_dir, 'RX_Matlab_Result_Images_png', 'DFE');
if ~exist(image_save_path_dfe, 'dir'), mkdir(image_save_path_dfe); end

% --- [7.1] DFE 参数配置 (使用顶部配置的全局参数) ---
...
% --- [7.2] 训练模式 (Training Mode) ---
fprintf('正在训练 DFE (Samples: %d)...\n', length(train_input_dfe));

N_train = length(train_input_dfe);
...
for k = 1 : N_train
    ...
    if k < nFfeTaps
        input_slice = [train_input_dfe(k:-1:1); zeros(nFfeTaps-k, 1)];
    else
        input_slice = train_input_dfe(k : -1 : k-nFfeTaps+1);
    end
    
    % 2. 理想符号
    des_val = train_label_dfe(k);
...
% 绘制收敛曲线
% h_conv = figure('Name', 'DFE Training Convergence');
% plot(10*log10(movmean(mse_history, 1000)));
% title('DFE Training MSE (dB)'); xlabel('Iteration'); ylabel('MSE (dB)'); grid on;
% fprintf('DFE 训练结束。最终 MSE: %.4f\n', mean(mse_history(end-5000:end)));

% --- [7.3] 测试模式 (Decision Directed Mode) ---
fprintf('正在均衡测试集 (Decision Directed)... \n');

N_test = length(test_input_dfe);
received_eq_dfe = zeros(1, N_test); % 保存为 received_eq_dfe
...
for k = 1 : N_test
    % 1. 准备 FFE 输入
    if k < nFfeTaps
        input_slice = [test_input_dfe(k:-1:1); zeros(nFfeTaps-k, 1)];
    else
        input_slice = test_input_dfe(k : -1 : k-nFfeTaps+1);
    end
...
disp('所有均衡器处理完毕，准备进入后处理阶段...');

%% === 8. 循环后处理与性能评估 (Unified Post-Processing) ===
% 定义要处理的模型列表
model_list = {'Volterra', 'DFE'};

for m_idx = 1:length(model_list)
    current_model_name = model_list{m_idx};
    ...
    % --- [8.1] 准备当前循环的数据与路径 ---
    if strcmp(current_model_name, 'Volterra')
        received_eq = received_eq_volterra; % 获取 Volterra 的结果
        % 路径设置
        curr_image_save_path = fullfile(root_base_dir, 'RX_Matlab_Result_Images_png', 'Volterra');
        curr_report_save_path = fullfile(root_base_dir, 'RX_Matlab_Result_Reports_txt', 'Volterra');
    else
        received_eq = received_eq_dfe;      % 获取 DFE 的结果
        % 路径设置
        curr_image_save_path = fullfile(root_base_dir, 'RX_Matlab_Result_Images_png', 'DFE');
        curr_report_save_path = fullfile(root_base_dir, 'RX_Matlab_Result_Reports_txt', 'DFE');
    end
    
    % 确保路径存在
    if ~exist(curr_image_save_path, 'dir'), mkdir(curr_image_save_path); end
    if ~exist(curr_report_save_path, 'dir'), mkdir(curr_report_save_path); end
    
    % =======================================================
    % 绘制并保存均衡后的 PAM4 眼图
    % =======================================================
    disp(['正在绘制 ', current_model_name, ' 均衡后眼图...']);
    
    % 1. 确保信号是列向量
    sig_for_eye = received_eq(:);
    
    % 2. 绘制眼图 (取前 2000 个符号，每迹显示 2 个符号周期)
    % 注意：均衡后的信号通常是 1 sps (1 sample per symbol)，所以周期设为 2 即可显示两个“眼睛”
    h_eye_eq = eyediagram(sig_for_eye(1:min(2000, length(sig_for_eye))), 2);
    set(h_eye_eq, 'Name', ['Equalized Eye - ' current_model_name]); % 设置窗口标题
    
    % 3. 保存图片
    eye_filename = sprintf('Eye_Eq_%s_%s_%s_%ddBm.png', ...
        prbs_base_name, run_mode, current_model_name, received_optical_power);
    full_eye_path = fullfile(curr_image_save_path, eye_filename);
    
    % 使用 gcf 获取当前眼图句柄并保存
    saveas(gcf, full_eye_path);
    fprintf('>>> 均衡后眼图已保存: %s\n', eye_filename);
    % =======================================================

    % --- [8.2] PAM4 判决 (Hard Decision) ---
    ...
    fprintf('>>> [%s] SER计算对齐: 检测到 Lag = %d\n', current_model_name, best_lag_ser);
    ...
    % 性能优化：仅使用前 1000 个点进行快速评估
        n_test = min(n_samples, 1000); % 快速评估
    ...
    if isrow(pcm_recovered), pcm_recovered = pcm_recovered.'; end 

    % --- [8.5] 计算 SQNR 与 物理对齐修正 ---
    ...
    ref_pcm_tx = OFDM(1 : R : end); % 降采样得到参考 PCM
    ...
    % 计算 SQNR
    len_rx_avail = length(pcm_recovered);
    len_ref_avail = length(ref_pcm_tx);
    len_common = min(len_rx_avail, len_ref_avail);
    
    if len_common > 0
        sig_rec = pcm_recovered(1 : len_common);
        sig_ref = ref_pcm_tx(1 : len_common);
    ...
    target_total_len = SymPerCar * one_symbol_len_samples;
    current_len = length(re_OFDM);
    % 强制长度匹配 (截断或补零)
    if current_len < target_total_len
        re_OFDM = [re_OFDM, zeros(1, target_total_len - current_len)];
    ...
    time_axis_sym = symLen * dt;
    cos_carrier = cos(2 * pi * fc * time_axis_sym);
    for k = 1:SymPerCar
    ...
    % --- [8.10] 绘图与保存 ---
    h_const = figure('Name', ['OFDM Constellation - ' current_model_name]);
    ...
    title(sprintf('[%s] After Correction\nEVM: %.2f%%', current_model_name, rmsEVM));
    ...
    const_filename = sprintf('Constellation_%s_%s_%s_%ddBm.png', ...
        prbs_base_name, run_mode, current_model_name, received_optical_power);
    full_const_path = fullfile(curr_image_save_path, const_filename);
    ...
    report_filename = sprintf('Report_%s_%s_%s_%ddBm.txt', ...
        prbs_base_name, run_mode, current_model_name, received_optical_power);
    full_report_path = fullfile(curr_report_save_path, report_filename);
    ...
    report_lines{end+1} = sprintf('Model Type    : %s', current_model_name);
