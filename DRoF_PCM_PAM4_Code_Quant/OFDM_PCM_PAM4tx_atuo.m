...
% --- 2. 选择本次要跑的 PRBS 文件名 ---
prbs_select_name = 'PRBS23.txt';      % 例如 'PRBS15.txt', 'PRBS23.txt', 'PRBS31.txt'
...
run_mode = 'train';      % <--- 在这里手动设置为 'train' 或 'test'

% --- 6. 手动设置量化比特数 (3 ~ 12 均可) ---
quant = 3; 

% --- 7. 路径自动适配 quant ---
param_save_path = sprintf('D:\\paperwork\\Experiment_Data\\Quant\\20Gsyms_30km_%dbit\\TX_Matlab_Param_mat', quant); % 参数文件的目标保存目录
vpi_save_path   = sprintf('D:\\paperwork\\Experiment_Data\\Quant\\20Gsyms_30km_%dbit\\VPI_Input_Data_txt', quant);  % VPI 波形文件 (.txt) 保存位置

% 自动创建目录
if ~exist(param_save_path, 'dir'), mkdir(param_save_path); end
...
% 这里选择 sps = 3 来获得一个 20 Gsym/s 的高速率。
...
Rs_PAM_final = Fs_AWG / sps; % 60e9 / 3 = 20 Gsym/s
...
% --------------------------------------------------------
% [关键策略] 固定 PCM 采样率 fb，不随 quant 变化
% --------------------------------------------------------
% 支持任意 quant (包括奇数)。
% 这意味着当 quant 变化时，实际的物理信息速率会变化，导致 Rx 端如果不重采样就会错位。
% 选 fb = 5e9 (5 GSa/s)。
% 原因：
%  a) 信号带宽 ~1.2GHz，5G > 2*1.2G，满足奈奎斯特。
% 带通采样频率fb必须大于信号带宽(BandpassBw)的两倍
%  b) Fs = 20G，fb = 5G -> R = 4 (整数!)。
% --------------------------------------------------------
fb = 5e9;  % 带通采样频率/PCM的采样率

% 3. 计算带通采样倍数  R  PCM带通采样的降采样因子
R = Fs / fb; % 20e9 / 5e9 = 4
...
%  OFDM信号带宽 (BandpassBw) 必须 <= fb/2 (即 2.5 GHz)
%  BandpassBw = Rs * (1 + Rolloff) = Rs * 1.2
%  Rs * 1.2 <= 2.5e9  => Rs 必须 <= 2.08 Gsym/s
Rs = 1e9;        % 设定：QAM基带符号速率 (1.0 Gsym/s) 

% 5. 采样率比率 (必须为整数)
Rate = Fs / Rs;       % OFDM上采样倍率 (20e9 / 1e9 = 20)
...
% =================== Bandpass sample ===============================
samplenumber = floor(length(OFDM)/R); % 使用 floor 防止索引越界
...
quant_Bin=de2bi(quant_Dec, quant, 'left-msb');    % dec to binary

% 【重点！！】
% 无论 quant 是奇数还是偶数，都不进行任何人为对齐填充。
% 直接将比特流串联。这意味着对于奇数比特，物理层符号可能不整除，这是符合物理规律的。
NRZ_code=reshape(quant_Bin',samplenumber*quant,1);% NRZ Tx

% = 检查点 =
% 此时 NRZ_code 速率 = fb * quant。
% 如果 quant=7, 速率 = 5G * 7 = 35 Gbps.

%% ==================== PAM4 coding =============================
% 如果总比特数 NRZ_code 长度是奇数，无法被 PAM4 (2bit) 整除，
% 我们只在整个流的最后补一个 0 (Stream Padding)，这不影响数据结构，只是为了生成波形。
if mod(length(NRZ_code), Mm) ~= 0
    NRZ_code = [NRZ_code; 0];
    fprintf('Notice: Total bits odd, padded 1 bit at the end of stream for PAM4.\n');
end
...
% = 检查点 =
% up_PAM 采样率 = PAM_preE速率 * R_AWG = 20 Gsym/s * 3 = 60 GSa/s.
rcos_PAM=conv(up_PAM,RRCtx);
% rcos_PAM 采样率也是 60 GSa/s.
eyediagram(rcos_PAM(:,15000:20000),sps)

%% 手动补零以匹配 VPI TimeWindow
% 1. 定义与 VPI TimeWindow 一致的总点数
total_sim_points = 3000000; % 必须与 VPI TimeWindow 匹配
% 2. 手动在 rcos_PAM 后补零
current_len = length(rcos_PAM); 
...
DRoF_PCM_Parameters.rcos_PAM  = rcos_PAM;  % 直接保存原始的、未补零的 rcos_PAM，因为 Rx 的 .mat 文件不需要补零！！
DRoF_PCM_Parameters.FFE_Taps = Tap3;
DRoF_PCM_Parameters.Fs_AWG = Fs_AWG;

% 构造 .mat 文件名 (例如 'DRoF_PCM_Parameters_PRBS23_8bit_train.mat')
param_filename = sprintf('DRoF_PCM_Parameters_%s_%dbit%s.mat', prbs_base_name, quant, file_suffix);
% 组合完整路径
...
% 组合成文件名 (例如 'PRBS23_PAM4_TX_20Gsym_8bit_train.txt')
% 注意：这里 rate_str_Gsym 是基于 Rs_PAM_final 计算的
rate_str_Gsym = Rs_PAM_final / 1e9; 
vpi_filename_only = sprintf('%s_%s_TX_%.0fGsym_%dbit%s.txt', prbs_base_name, pam_part, rate_str_Gsym, quant, file_suffix);
