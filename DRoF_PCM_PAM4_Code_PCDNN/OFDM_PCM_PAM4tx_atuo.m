...
% --- 6. 设置参数文件保存位置 ---
% 设置参数文件的目标保存目录
param_save_path = fullfile(root_base_dir, 'TX_Matlab_Param_mat');
...
vpi_save_path = fullfile(root_base_dir, 'VPI_Input_Data_txt');
...
%
%【】【】【】【】【】【】【】【】【】【】【】【】【】
Rs = 1.25e9;               % 设定：QAM基带符号速率 (1.6 Gsym/s) (<= 1.667e9)
...
% --- 同步头（改为专用伪随机同步序列，避免与业务数据发生伪匹配） ---
sync_guard_zeros = 2000;   % 前导保护零
sync_tail_zeros = 2;       % 同步后过渡零
sync_len = 1024;           % 专用同步序列长度（符号）
sync_rng_seed = 20260308;  % 固定随机种子，保证收发完全一致

rng(sync_rng_seed, 'twister');
sync_seq = randi([0, MM-1], 1, sync_len);  % 符号域取值 [0,1,2,3]

PAM_code2 = [sync_seq, zeros(1, sync_tail_zeros), PAM_code];
rcos_PAM = [zeros(1, sync_guard_zeros), PAM_code2];	% 在最前面添加保护间隔
...
% 记录同步段在FFE输出后的参考符号（用于Rx符号级同步模板）
sync_start_ffe = sync_guard_zeros + 1;
sync_end_ffe = sync_guard_zeros + sync_len + sync_tail_zeros;
sync_ref_after_ffe = PAM_preE(sync_start_ffe:sync_end_ffe);
...
total_sim_points = 2000000; % 必须与 VPI TimeWindow (200000/64e9) 匹配
...
DRoF_PCM_Parameters.PCM_bp_OFDM = bp_OFDM; % 量化前的PCM带通采样点(用于NN PCM回归标签)
DRoF_PCM_Parameters.PAM_MM  = MM;
DRoF_PCM_Parameters.PAM_Mm  = Mm;
DRoF_PCM_Parameters.PAM_code  = PAM_code;
DRoF_PCM_Parameters.SYNC_seq = sync_seq;
DRoF_PCM_Parameters.SYNC_len = sync_len;
DRoF_PCM_Parameters.SYNC_guard_zeros = sync_guard_zeros;
DRoF_PCM_Parameters.SYNC_tail_zeros = sync_tail_zeros;
DRoF_PCM_Parameters.SYNC_rng_seed = sync_rng_seed;
DRoF_PCM_Parameters.SYNC_ref_after_ffe = sync_ref_after_ffe;
...
% 构造 .mat 文件名 (例如 'DRoF_PCM_Parameters_PRBS23_train.mat')
...
% 组合成文件名 (例如 'PRBS23_PAM4_TX_20Gsym_train.txt')
